...
export const AppProvider = ({ children }) => {
  ...
  const loadProducts = async () => {
    try {
      ...
      const data = await Promise.race([fetchProductsAPI(), timeout]);
  ...
  useEffect(() => {
    if (user.id) {
      AsyncStorage.setItem(`wishlist_${user.id}`, JSON.stringify(wishlist)).catch(() => {});
    }
  }, [wishlist, user.id]);
  ...
  useEffect(() => {
    if (user.id) {
      AsyncStorage.setItem(`orders_${user.id}`, JSON.stringify(orders)).catch(() => {});
    }
  }, [orders, user.id]);
  ...
  const loadAuthState = async () => {
    try {
      ...
      if (savedAuth) {
        ...
        setIsLoggedIn(true);
        if (savedUser?.id) {
          const [wl, ord] = await Promise.all([
            AsyncStorage.getItem(`wishlist_${savedUser.id}`),
            AsyncStorage.getItem(`orders_${savedUser.id}`),
          ]);
  ...
  // Auth functions
  const login = async (type, userData = null, authToken = null, newUser = false) => {
    setIsLoggedIn(true);
    setUserType(type);
    ...
    // Load user-specific wishlist and orders
    if (type === 'customer' && userData?.id) {
      try {
        const [wl, ord] = await Promise.all([
          AsyncStorage.getItem(`wishlist_${userData.id}`),
          AsyncStorage.getItem(`orders_${userData.id}`),
        ]);
        if (wl) setWishlist(JSON.parse(wl));
        if (ord) setOrders(JSON.parse(ord));
      } catch (_) {}
